...
class Conv2dBNReLU(_FoldModule):
    ...
    def __init__(self, conv, bn, relu):
        super().__init__()
        if isinstance(conv, nn.Conv2d) and isinstance(bn, nn.BatchNorm2d) and isinstance(relu, nn.ReLU):
            # weight / bias / activation
            self.layer_data_type = {"weight": True, "bias": True, "acti": True}
            self.conv = conv
            self.bn = bn
            self.relu = relu
    ...
    def set_pim_unit(self, pim_unit):
        ...
        self.relu.set_pim_unit(pim_unit)
    ...
    def forward(self, input):
        ...
        if not PIMUnit.quant_phase == PIMUnit.QuantPhase.FLOAT and \
           self.bn_frozen and \
           not self.last:
