...
void BasicDecoder::Initialize(int _numAddressBit, double _capLoad, double _resLoad, bool _normal){
    /*if (initialized)
    ...
    if (numNandInput == 0) {
        ...
        outputDriver.Initialize(logicEffortInv, capInv, capLoad, resLoad, capLoad * resLoad / 2, true, latency_first, 0);  /* Always Latency First */
    }
    else{
        ...
        outputDriver.Initialize(logicEffortNand, capNand, capLoad, resLoad, capLoad * resLoad / 2, true, latency_first, 0);  /* Always Latency First */
    }
    initialized = true;
}
