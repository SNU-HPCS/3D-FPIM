...
void Wire::ReadWireParameterFromFile(const std::string & inputFile) {
    ...
    capWirePerUnit = -1;
    resWirePerCell = -1;
    resWirePerStaircase = -1;

    while (fscanf(fp, "%[^\n]\n", line) != EOF) {
        if (!strncmp("-resWirePerUnit", line, strlen("-resWirePerUnit"))) {
            sscanf(line, "-resWirePerUnit: %lf", &resWirePerUnit);
            continue;
        }
        if (!strncmp("-capWirePerUnit", line, strlen("-capWirePerUnit"))) {
            sscanf(line, "-capWirePerUnit: %lf", &capWirePerUnit);
            continue;
        }
        //////////////////////////////////////////////////////////////////////
        if (!strncmp("-resWirePerCell", line, strlen("-resWirePerCell"))) {
            sscanf(line, "-resWirePerCell: %lf", &resWirePerCell);
            continue;
        }
        if (!strncmp("-resWirePerStaircase", line, strlen("-resWirePerStaircase"))) {
            sscanf(line, "-resWirePerStaircase: %lf", &resWirePerStaircase);
            continue;
        }
    }
}
...
void Wire::Initialize
    (int _featureSizeInNano, WireType _wireType, 
    WireRepeaterType _wireRepeaterType, int _temperature, bool _isLowSwing) {
    ...
    if(wireType == bitline_wire){
        wirePitch = 39e-9;
        copper_resistivity = 9e-8;
        ...
        // obtained from CACTI
        double alphaScatter = 1.;
        double barrierThickness = 3e-9;
        double dishingThickness = 19e-9;

        resWirePerUnit = CalculateWireResistance
            (copper_resistivity, wireWidth, wireThickness, 
                barrierThickness,
                dishingThickness, alphaScatter);

        intrinsicCapWirePerUnit = CalculateIntrinsicWireCapacitance
            (PERMITTIVITY, wireWidth, ildThickness, SIO2EPS, 1.15e-10);

        // We set the miller value to 1 => they always change in the same direction
        couplingCapWirePerUnit = CalculateCouplingWireCapacitance
            (PERMITTIVITY, wireThickness, wireSpacing, 1, SIO2EPS);
        capWirePerUnit = intrinsicCapWirePerUnit + couplingCapWirePerUnit;

        capViaWirePerString = 0.;
        capViaWirePerString += CalculateVerticalCouplingCapacitance 
            (PERMITTIVITY, 81e-9 / 2, 150e-9, 1, SIO2EPS) * 850e-9 * 6 /* there are two sets nearby */;
        ...
        capWirePerUnit = CalculateWireCapacitance
            ...
            ildThickness, 1.5 /* miller value */, 
            horizontalDielectric, SIO2EPS /* Vertical Dielectric */,
            1.15e-10);
    }
...
Wire & Wire::operator=(const Wire &rhs) {
    ...
    intrinsicCapWirePerUnit = rhs.intrinsicCapWirePerUnit;
    couplingCapWirePerUnit = rhs.couplingCapWirePerUnit;
    resWirePerCell = rhs.resWirePerCell;
    resWirePerStaircase = rhs.resWirePerStaircase;
